...
Brighten(double factor)
{
  ...
  // then clamping the result to a valid range.
	...
	for (int j = 0; j < height; j++) {
    	for (int i = 0; i < width; i++) {
      		R2Pixel& pixel = Pixel(i, j);
      		pixel[0] *= factor;            // increasing brightness of all three channel by multiplying with factor.
      		pixel[1] *= factor;
      		pixel[2] *= factor;
      		pixel[0] = std::max(l,std::min(pixel[0],h)); //clipping all three channel between 0,1
      		pixel[1] = std::max(l,std::min(pixel[1],h));
      		pixel[2] = std::max(l,std::min(pixel[2],h));
      		pixel.Clamp();
    }
  }
  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "Brighten(%g) is implemented\n", factor);
}
...
ChangeContrast(double factor)
{
  ...
  // and negative factors generate inverted images.
    ...
    //cal average luminance of all pixels
    double lum=0;
    for (int i = 0; i < npixels; i++) {
		lum += pixels[i].Luminance();
	}
	lum /= npixels;
	R2Pixel greyImg(lum, lum, lum, 1.0); //grey image conversion.
  
	for (int i = 0; i < npixels; i++) { //adding weight to each pixel
		pixels[i] = (1-factor)*greyImg + factor*pixels[i];
		pixels[i].Clamp();
    }

  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "ChangeContrast(%g) implemented\n", factor);
}
...
Blur(double sigma)
{
    ...
    for (int i = 0; i < npixels; i++) {               //adding opaqueness by alpha 2.0
		pixels[i].SetRed(pow(pixels[i].Red(), 2.0));
		pixels[i].SetGreen(pow(pixels[i].Green(), 2.0));
		pixels[i].SetBlue(pow(pixels[i].Blue(), 2.0));
  	}
  	R2Image original(*this);

    int size = std::ceil(3*sigma)*2+1;
	if(size < 1) size = 1;
	std::vector<double> col(size+1, 0.0);
    std::vector<std::vector<double> > Gkernel(size+1, col); // initializing kernel of mentioned size
    int i,j;

    for (i=0 ; i<=size ; i++) {         
        for (j=0 ; j<=size ; j++) {                            //calculating gaussian kernal filter
            Gkernel[i][j] = exp(-(i*i+j*j)/(2*sigma*sigma));
        }
    }


    for (int i = 0; i < npixels; i++) {
		R2Pixel pix(0.0, 0.0, 0.0, pixels[i].Alpha());
		int ww = i/height;
		int hh = i%height;
		double sum=0;
                          //convolving each pixel of image with gaussian kernel
		for(int j = (ww - size < 0 ? 0 : ww - size); j < (ww + size + 1 > width ? width : ww + size + 1); j++) {
			for(int k = (hh - size < 0 ? 0 : hh - size); k < (hh + size + 1 > height ? height : hh + size + 1); k++) {
				double gus = Gkernel[abs(j-ww)][abs(k-hh)];
				pix += gus*original.Pixel(j,k);
				sum += gus;
			}
		}
		pix /= sum;
  ...
  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "Blur(%g) implemented\n", sigma);
}
...
Sharpen()
{
  ...
  double factor=2.0;
  R2Image ImgBlur(*this);
  ImgBlur.Blur(factor);  //blurring the image using gaussian blur function

  for (int i = 0; i < npixels; i++) { // adding weight to the pixel of image by factor.
	pixels[i] = (1-factor)*ImgBlur.Pixels()[i] + factor*pixels[i];
	pixels[i].Clamp();
  }
...
EdgeDetect(void)
{
    ...
    R2Image orig(*this);
	//finding edges by convolving each pixel with filter
	for (int i = 0; i < npixels; i++) {
		int ww = i/height;
		int hh = i%height;
		R2Pixel pix(0.0, 0.0, 0.0, 1.0);
		double sum = 0;

		for(int j = ww - 1; j <= ww + 1; j++) {
			if(j < 0 || j >= width) continue;
			for(int k = hh - 1; k <= hh + 1; k++) {
				if(k < 0 || k >= height) continue;

				if(j == ww && k == hh) {
					pix += 8*orig.Pixel(j,k);
					sum += 8;
				} else {
					pix += -1.0*orig.Pixel(j,k);
					sum++;
				}
...
R2Pixel R2Image::Sample(double ww, double hh, int sampling_method, double sigma_x, double sigma_y)
{
	if(sampling_method == R2_IMAGE_POINT_SAMPLING) {  //point sampling 
		int x_img = lround(ww);   // long round of values
		int y_img = lround(hh);

		if(x_img < 0) x_img = 0;           //checking the boundary conditions
		if(x_img >= width) x_img = width - 1;
		if(y_img < 0) y_img = 0;
		if(y_img >= height) y_img = height - 1;
		return Pixel(x_img, y_img);
	}
...
Scale(double sx, double sy, int sampling_method)
{
  // Scale an image in x by sx, and y by sy.
    R2Image img(*this);               //cloning image
	width = lround(sx*img.width);      //getting new width and height
	height = lround(sy*img.height);

	delete [] pixels;        // pixel destroyer
	npixels = width*height;  // size of new image
	pixels = new R2Pixel[npixels];

	double x_offset = 0.5 * ((double)img.width) / ((double)width) - 0.5;   //getting offset along x and y axis
	double y_offset = 0.5 * ((double)img.height) / ((double)height) - 0.5;
	
	for(int i=0; i<npixels; i++) {
		int ww = i/height;
		int hh = i%height;

		double x_img = ((double)img.width) / ((double)width) * ww + x_offset;     //adding offset to new pixel for resizing
		double y_img = ((double)img.height) / ((double)height) * hh + y_offset; 

		double sigma_x=0.5, sigma_y=0.5;
		pixels[i] = img.Sample(x_img, y_img, sampling_method,sigma_x, sigma_y);
	}


  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "Scale(%g, %g, %d) implemented\n", sx, sy, sampling_method);
}
...
Composite(const R2Image& top, int operation)
{
  // Composite passed image on top of this one using operation (e.g., OVER)
	for(int i=0; i<npixels; i++) {                       //alpha bending is used to over lay transpaerent image over back ground image.
		double alpha_top = top.pixels[i].Alpha();  //  getting alpha channel of top image
		double alpha_bottom = pixels[i].Alpha();// getting alph achannel of background image

		pixels[i] = alpha_top * top.pixels[i] + alpha_bottom * (1 - alpha_top) * pixels[i];  //combine the foreground image color  
		pixels[i].SetAlpha(alpha_top + alpha_bottom * (1-alpha_top)); //and the background image using the alpha mask.
	}
  //the boundary of the alpha mask usually has pixels that are between 0 and 1 alpha. 0 -> output pixel color is the background and 1-> for foreground
  // Fill IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  fprintf(stderr, "Composite implemented\n");
}				
